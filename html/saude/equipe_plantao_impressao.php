...
foreach ($escala['dias'] ?? [] as $diaEscala) {
    ...
    foreach ($turnos as $turno) {
        ...
        if (!empty($dadosTurno['id_equipe_plantao']) && $equipeAtiva !== 1) {
            ...
            $dadosTurno['equipe_nome'] = 'Nao definida';
...
<head>
    <meta charset="UTF-8">
    <title>Impressao da Escala de Plantao</title>
...
<div class="print-wrap">
    <div class="print-toolbar">
        ...
        <div class="right">
            <span class="meta">Periodo: <?php echo htmlspecialchars($mesNome . '/' . $ano, ENT_QUOTES, 'UTF-8'); ?></span>
        </div>
    </div>

    <header class="print-title">
        <div>
            <h1>Escala Mensal de Plantao</h1>
            <p>Modulo Saude - Planejamento Operacional</p>
            <p>Periodo: <?php echo htmlspecialchars($mesNome . '/' . $ano, ENT_QUOTES, 'UTF-8'); ?> | Emissao: <?php echo htmlspecialchars($emissao, ENT_QUOTES, 'UTF-8'); ?></p>
        </div>
        <?php if ($logoPath !== ''): ?>
    ...
    <section class="info-grid">
        <div class="info-card">
            <div class="label">Dias do mes</div>
            <div class="value"><?php echo (int) $totalDiasMes; ?></div>
        </div>
        <div class="info-card">
            <div class="label">Plantoes preenchidos</div>
            <div class="value"><?php echo (int) $preenchidos; ?></div>
        </div>
        <div class="info-card">
            <div class="label">Plantoes pendentes</div>
    ...
    <?php if (!empty($escala['observacao'])): ?>
        <section class="info-card" style="margin-bottom: 12px;">
            <div class="label">Observacao geral da escala</div>
    ...
    <section class="legend">
        <?php if (empty($equipesUsadas)): ?>
            <span class="legend-item">Nenhuma equipe definida no periodo.</span>
        <?php else: ?>
            <?php foreach ($equipesUsadas as $dadosEquipe): ?>
                <span class="legend-item">
                    ...
                    (<?php echo (int) $dadosEquipe['plantoes']; ?> plantoes)
                </span>
            <?php endforeach; ?>
        <?php endif; ?>
    </section>
                            ...
                            <?php foreach ($turnos as $turno): ?>
                                ...
                                <div class="print-shift-block">
                                    ...
                                    <span class="day-team" style="background: <?php echo htmlspecialchars($corEquipe, ENT_QUOTES, 'UTF-8'); ?>; color: <?php echo htmlspecialchars($corTextoEquipe, ENT_QUOTES, 'UTF-8'); ?>;">
                                        <?php echo htmlspecialchars((string) ($dadosTurno['equipe_nome'] ?? 'Nao definida'), ENT_QUOTES, 'UTF-8'); ?>
                                    </span>
            ...
            <?php foreach ($mapaDias as $dia => $infoDia): ?>
                <?php foreach ($turnos as $turno): ?>
                    ...
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars((string) ($dadosTurno['equipe_nome'] ?? 'Nao definida'), ENT_QUOTES, 'UTF-8'); ?></td>
    ...
    <section class="signature-area">
        ...
        <div class="signature-line">Conferencia / supervisao</div>
    </section>
</div>
