	...
	public function listarTodos($id_memorando)
	{
		try {
			...
			while ($linha = $consulta->fetch(PDO::FETCH_ASSOC) and $linha1 = $consulta1->fetch(PDO::FETCH_ASSOC)) {
				$Despachos[$x] = array('remetente' => htmlspecialchars($linha['nome']), 'data' => $linha['data'], 'texto' => $linha['texto'], 'id' => $linha['id_despacho'], 'destinatario' => htmlspecialchars($linha1['nome']));
				$x++;
			}
