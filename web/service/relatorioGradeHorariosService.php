...
try {
    $pdo = Conexao::connect();
    if (!$pdo) throw new Exception("Não foi possível estabelecer conexão com o banco.");
    ...
    if ($idAgenda <= 0) {
        ...
        if (empty($agendas)) throw new Exception("Nenhuma agenda encontrada.");
    ...
    if (empty($equipes)) throw new Exception("Nenhuma equipe vinculada a esta agenda.");
    ...
    foreach ($equipes as $eq) {
        ...
        $textoHorario = isset($horariosEquipe[$eq['id']]) ? " <strong style='color:#5DADE2;'>{$horariosEquipe[$eq['id']]}</strong>" : "";
    ...
    foreach ($alocacoes as $evento) {
        ...
        if ($evStart->format('m') == $mesAlvo && $evStart->format('Y') == $anoAlvo) {
            ...
            if ($d >= 1 && $d <= $numeroDias) {
                ...
                $eventosPorDia[$d][] = "<div style='color:#000000; font-weight:bold; font-size: 8.5pt; line-height: 1.1;'>{$nomeEq}</div><div style='font-size: 4pt; line-height: 4pt;'>&nbsp;</div><div style='color:#333333; font-size: 6.5pt; line-height: 1.2;'>({$membrosEq}) - <strong style='color:#1a365d;'>{$rotuloTurno}</strong></div>";
            }
        }
    }
    ...
    $mpdf = new Mpdf([
        'format' => 'A4-L',
        'orientation' => 'L',
        'margin_left' => 6, 'margin_right' => 6, 'margin_top' => 6, 'margin_bottom' => 6,
        'tempDir' => $tempDir
    ]);

    $css = "body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333333; } .header-container { text-align: center; padding-bottom: 6px; margin-bottom: 8px; } h2 { margin: 0; font-size: 15pt; color: #333333; text-transform: uppercase; letter-spacing: 1px; } h3 { margin: 3px 0 0 0; font-size: 11pt; color: #666666; font-weight: normal; } .equipes-legenda { font-size: 7.5pt; background-color: #f5f5f5; padding: 4px; border: 1px solid #d9d9d9; border-radius: 4px; margin-bottom: 8px; text-align: center; line-height: 1.4; } table.calendario { width: 100%; border-collapse: collapse; table-layout: fixed; } table.calendario th { background-color: #6b6b6b; color: #ffffff; font-weight: bold; font-size: 9pt; padding: 6px 4px; width: 14.28%; text-transform: uppercase; border: 1px solid #6b6b6b; } table.calendario td { border: 1px solid #cfcfcf; height: 110px; vertical-align: top; padding: 5px; font-size: 7pt; } .dia-numero { font-weight: bold; font-size: 8pt; display: block; margin-bottom: 6px; color: #444444; background-color: #eeeeee; padding: 2px 4px; border-radius: 2px; text-align: left; width: 14px; } .hoje-vazio { background-color: #f7f7f7; border: 1px solid #eeeeee; } .fim-semana { background-color: #fafafa; } .fim-semana .dia-numero { color: #555555; background-color: #e6e6e6; } .event-item { padding: 0; margin: 0; }";
    ...
    $html = "<div class='header-container'><h2>Agenda Mensal - {$nomeAgenda}</h2><h3>" . $mesesNomes[$mesAlvo] . " de " . e((string)$anoAlvo) . "</h3></div><div class='equipes-legenda'>" . implode(' &nbsp;|&nbsp; ', $legendasEquipes) . "</div><table class='calendario'><thead><tr><th>Domingo</th><th>Segunda</th><th>Terça</th><th>Quarta</th><th>Quinta</th><th>Sexta</th><th>Sábado</th></tr></thead><tbody><tr>";
    
    for ($i = 0; $i < $diaDaSemanaInicio; $i++) { $html .= "<td class='hoje-vazio'></td>"; }
    ...
    while ($diaAtual <= $numeroDias) {
        ...
        $diaAtual++;
        $diaDaSemanaRodando++;
        if ($diaDaSemanaRodando > 6 && $diaAtual <= $numeroDias) { $html .= "</tr><tr>"; $diaDaSemanaRodando = 0; }
    }
    while ($diaDaSemanaRodando <= 6) { $html .= "<td class='hoje-vazio'></td>"; $diaDaSemanaRodando++; }
    $html .= "</tr></tbody></table>";
    
    $usuario = isset($_SESSION['nome']) ? e($_SESSION['nome']) : 'Usuário';
    $html .= "<div style='margin-top: 25px; width: 100%;'><table style='width:100%; border-collapse: collapse;'><tr><td style='width:50%; text-align:left; font-size:9pt;'><strong>Feito por:</strong> {$usuario}</td><td style='width:50%; text-align:center; font-size:9pt;'>___________________________________<br>Assinatura</td></tr></table></div>";

    $mpdf->WriteHTML($html, HTMLParserMode::HTML_BODY);
    $mpdf->Output("agenda_mensal_{$anoAlvo}_{$mesAlvo}.pdf", 'I');

} catch (Exception $e) {
    echo "<div style='font-family: sans-serif; text-align: center; margin-top: 50px;'><h1 style='color: #e74c3c;'>Erro ao gerar agenda</h1><p style='color: #333;'>" . e($e->getMessage()) . "</p></div>";
}