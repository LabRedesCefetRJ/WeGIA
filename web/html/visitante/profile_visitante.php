...
if (!$idVisitante || $idVisitante < 1) {
    echo json_encode(['erro' => 'O ID de um visitante deve ser maior ou igual a 1.']);
    exit(400);
}
...
try {
    ...
    if (!isset($_SESSION['visitante'])) {
        header('Location: ../../controle/control.php?metodo=listarUm&nomeClasse=VisitanteControle&id_visitante=' . urlencode($idVisitante));
        exit();
    } else {
        $visitante = $_SESSION['visitante'];
        unset($_SESSION['visitante']);
        $visitante = json_decode($visitante);
        if ($visitante) {
            $visitante = $visitante[0];
            $visitante = json_encode([$visitante]);
        }
    }
...
<head>
    ...
    <script>
        ...
        $(function() {
            ...
            $.each(visitante, function(i, item) {
                // Info Pessoal
                        ...
                        <section class="panel">
                            <div class="panel-body">
                                <div class="thumb-info mb-md">
                                    <?php
                                    $stmtImagem = $pdo->prepare("SELECT pessoa.imagem, pessoa.nome FROM pessoa, visitante WHERE pessoa.id_pessoa=visitante.id_pessoa and visitante.id_visitante=:idVisitante");
                                    $stmtImagem->bindValue(':idVisitante', $idVisitante, PDO::PARAM_INT);
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        <input type="hidden" name="id_visitante" value="<?= htmlspecialchars($idVisitante) ?>">
