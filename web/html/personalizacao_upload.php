...
<body>
    <div>
        ...
        if(!$idPessoa || $idPessoa < 1){
            ...
            header("Location: home.php?msg_c=Falta de permissão para acessar o recurso");
            exit();
        }

        //verificar permissão
        require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'permissao' . DIRECTORY_SEPARATOR . 'permissao.php';
        permissao($idPessoa, 9, 7);
        ...
        if (isset($_FILES['img_file']['name']) && $_FILES['img_file']['error'] == 0) {
            ...
            if ($id === 8 && !Util::validaCNPJ($txt)) {
                http_response_code(400);
                header("location: personalizacao.php?msg=error&err=CNPJ inválido");
                exit();
            }
